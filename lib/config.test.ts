...
describe('config', () => {
    ...
    it('no random ua', async () => {
        ...
        const { config } = await import('./config');
        expect(config.ua).toBe('openRSS/0.1 (+http://github.com/haojunyu/openRSS; like FeedFetcher-Google)');
    ...
    it('random ua', async () => {
        const { config } = await import('./config');
        expect(config.ua).not.toBe('openRSS/0.1 (+http://github.com/haojunyu/openRSS; like FeedFetcher-Google)');
    });
